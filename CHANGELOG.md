...
## v2.16 — Between-wave Upgrade Shop
- Clear a wave and the **UPGRADE SHOP** opens (the next-wave countdown freezes). Spend
  **SCORE** on per-run upgrades, then hit **DEPLOY** to start the next wave.
- Five upgrades, each multi-level with rising cost: VITALITY (+25 max HP & full heal),
  FIREPOWER (+15% weapon damage), FAST HANDS (-18% reload time), MUNITIONS (+30% ammo
  reserve, refilled), DEMOLITION (+1 grenade per wave).
- Real roguelite tension: spend score to survive deeper, or bank it for a higher final score.
- Works on desktop (pointer unlocks for the menu) and mobile (touch).
